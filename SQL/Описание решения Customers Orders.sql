...
SELECT CompanyName from Customers where CustomerID IN (SELECT CustomerID from Orders WHERE OrderDate < '1996-11-15' OR OrderDate > '1997-02-18'); или SELECT CompanyName from Customers where CustomerID IN (SELECT CustomerID from Orders WHERE OrderDate NOT BETWEEN '1996-11-15' AND '1997-02-18');																								
...
если дата 18.02.1997 включительно, то код будет Select * FROM Orders where OrderDate >='1996-12-12' AND OrderDate <= '1997-02-18';																								
...
SELECT orders.CustomerID, orders.OrderDate, Customers.CompanyName, Customers.Address, Customers.City, Customers.Country
FROM orders
JOIN Customers ON Orders.CustomerID = Customers.CustomerID;																								
